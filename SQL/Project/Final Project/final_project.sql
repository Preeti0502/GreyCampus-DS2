...
/*Postgresql does have "unsigned" if we simply use int instead of "unsigned int" it 
would work in same manner i.e, we get only postive integers. Also we cannot set size to
intger datatype, therefore to preserve the data type used we must prefer to omit the
size constraint or instead NUMERIC(11,0) or DECIMAL(11,0) could be used specifying size 
constraint.*/
...
/*Removing unsigned and size constrained on integer.*/
...
/* "Serial" is used to create unique identifier columns similar to AUTO-INCREMENT property
supported by some other databases. It creates an integer column and arranged for its default 
values to be assigned from a sequence generator. If using Serial we wont need to specify 
not null. It will be automatically applied.*/
...
/*Same, removing unsigned and size for integer type*/
...
/* Postgresql does not support "Unsigned". Skipping it would make no difference.*/
...
/*Postgresql automatically creates a unique index when a unique constraint is defined for
a table.The index covers the columns that make up the primary key or unique constraint. 
Also, there is no need to manually create indexs on unique columns as doing so would just duplicate the automatically-created
index.*/

ALTER TABLE job_history ADD UNIQUE(
	employee_id,
	start_date
	);
...
COMMIT;
/*NOTE: COMMIT commits the current transaction. All changes made by transaction become 
...
/* SET FOREIGN_KEY_CHECKS = 0, is useful when we need to re-create the tables and load 
data in any parent-child order. It temporarily disables refrential constraints. The 
command is not available in Postgresql we can enable and disable refrential key using
...
/*To conert string to date format we need to use To_DATE('str',format) */
...
SELECT first_name, last_name, salary, ROUND(((15*salary)/100),3) AS PF FROM employees;
...
SELECT employee_id, first_name, last_name, SUM(salary) AS "TOTAL SALARY"
FROM employees
...
SELECT round(AVG(salary),3) AS AVG_SALARY, COUNT(employee_id) AS NO_OF_EMP
FROM employees;

--9.	Write a query to get the number of employees working with the company.

SELECT COUNT(DISTINCT(employee_id)) AS NO_OF_EMP 
FROM employees;
...
--12.	Write a query to get first name from employees table after removing 
--white spaces from both side.

SELECT TRIM(first_name) AS first_name,LENGTH(TRIM(first_name)) FROM employees;

--13.	Write a query to get the length of the employee names (first_name, last_name) 
...
--14.	Write a query to display the name (first_name, last_name) and salary for all
...
--15. Write a query to display the name (first_name, last_name) and salary for all 
